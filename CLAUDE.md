...
## Run / build / test
- **Backend:** `uvicorn api.main:app --reload`. Env: `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `APP_ENCRYPTION_KEY` (Fernet — encrypts users' BYOK keys); optional `ALLOWED_ORIGINS`, `ADMIN_EMAILS`, `CRON_SECRET` (guards `/cron/*`), `GMAIL_ADDRESS` + `GMAIL_APP_PASSWORD` (digest email sender). **No shared `ANTHROPIC_API_KEY`** — Anthropic keys are per-user (BYOK). Python 3.11; deps pinned in `requirements.txt`. Lint: `ruff check .`.
...
- **Agreed next-features order** (re-ranked by value 2026-07-15, usage tracking added 2026-07-16): 1) contract/cap planner page — **shipped 2026-07-16** (the `/roster` rebuild), 2) daily digest — **shipped 2026-07-16**: `.github/workflows/daily-digest.yml` (16:00 UTC = 9am PDT) warms widgets then hits `/cron/daily-digest`, which emails each opted-in league owner (leagues.digest_enabled, Settings toggle) an AI "Lead" (one no-search Sonnet call, `tool="digest"`) + stitched cached widgets via Gmail SMTP. Needs secrets: GH `API_URL`+`CRON_SECRET`, Railway `CRON_SECRET`+`GMAIL_ADDRESS`+`GMAIL_APP_PASSWORD`. 3) AI usage/cost tracking — **capture layer shipped 2026-07-16** (`ai_usage` + proxy metering); remaining: admin rollups per tool/league/user with est. cost computed at display time from a pricing map (verify current model prices when building). 4) rank trends over time (auto-compute on sync + `category_rank_history` + sparklines), 5) trade outcome tracking on `trade_history`.
