...
- **Agreed next-features order** (re-ranked by value 2026-07-15, usage tracking added 2026-07-16): 1) contract/cap planner page — **shipped 2026-07-16** (the `/roster` rebuild), 2) daily digest via the `/cron/refresh-widgets` warmer (needs scheduled trigger + push/email), 3) AI usage/cost tracking — new `ai_usage` table (service-role only; league/owner/tool/model/token counters/web-search count/duration per call, no prompt content) written best-effort from every Anthropic call site (streams report usage in first+last events); admin rollups per tool/league/user with est. cost computed at display time from a pricing map (verify current model prices when building). **Capture layer should ship early even if the UI waits — data only accrues once live.** 4) rank trends over time (auto-compute on sync + `category_rank_history` + sparklines), 5) trade outcome tracking on `trade_history`.
